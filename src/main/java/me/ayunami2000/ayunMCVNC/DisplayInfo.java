			...
			if (Main.plugin.ffmpegEnabled) {
				try {
					if (Main.plugin.ffmpegParams.isEmpty()) {
						...
						audioProcess = new ProcessBuilder("ffmpeg", "-stream_loop", "-1", "-hide_banner", "-loglevel", "error", "-fflags", "nobuffer", "-async", Main.plugin.audioFrequency + "", "-thread_queue_size", "4096", "-f", sampleFormat, "-acodec", "pcm_" + sampleFormat, "-ac", Main.plugin.audioChannelNum + "", "-ar", Main.plugin.audioFrequency + "", "-i", "pipe:", "-f", "mp3", "-").start();
					} else {
